...
boxIds.every(thisBox => {
    const thisLetters = thisBox.split('');
    if (boxIds.every(otherBox => {
        if (thisBox == otherBox) {
            return true;
        }
        const otherLetters = otherBox.split('');
        let sameLetters = [];
        for (let letterIndex = 0; letterIndex < thisLetters.length; letterIndex++) {
            if (thisLetters[letterIndex] == otherLetters[letterIndex]) {
                sameLetters.push(thisLetters[letterIndex]);
            }
        }
        let same = sameLetters.join('');
        console.log(thisBox, otherBox, same, same.length, thisBox.length);

        if (same.length == thisBox.length - 1) {
